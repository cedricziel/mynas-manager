...
class TrueNasVersionManager implements IVersionManager {
  ...
  TrueNasVersionManager({required IJsonRpcClient jsonRpcClient})
  ...
  Future<TrueNasVersion> detectVersion() async {
    ...
    try {
      ...
      if (result is String) {
        ...
        throw Exception(
          'Unexpected version response type: ${result.runtimeType}',
        );
      }
  ...
  bool isFeatureSupported(String feature) {
    if (_currentVersion == null) {
      _logger.warning(
        'Version not detected, assuming feature is supported: $feature',
      );
      return true;
    }
  ...
  bool isVersionCompatible() {
    ...
    if (_currentVersion! < minVersion) {
      _logger.warning(
        'TrueNAS version $_currentVersion is below minimum supported version $minVersion',
      );
      return false;
    }
  ...
  Map<String, bool> getFeatureCapabilities() {
    ...
    return {
      'json_rpc_websocket': _currentVersion!.supportsFeature(
        'json_rpc_websocket',
      ),
  ...
  Map<String, dynamic> getRecommendedClientConfig() {
    ...
    return {
      ...
      'auth_methods': capabilities['enhanced_auth'] == true
          ? ['api_key', 'credentials']
          : ['credentials'],
      'supported_features': capabilities.keys
          .where((k) => capabilities[k] == true)
          .toList(),
    };
  }
}
