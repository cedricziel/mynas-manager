...
void main() {
  group('TrueNasPeerClient', () {
    ...
    setUp(() {
      ...
      when(mockConnectionManager.stateStream).thenAnswer(
        (_) => Stream.value(ConnectionState.connected)
      );
      when(mockConnectionManager.isConnected).thenReturn(true);
      when(mockConnectionManager.messageStream).thenAnswer(
        (_) => incomingController.stream
      );
    ...
    group('Parameter Transformation', () {
      test('should send empty array for methods requiring empty params', () async {
    ...
    group('Notifications', () {
      ...
      test('should handle collection_update notifications', () async {
        ...
        expect(notifications[0].params?['collection'], equals('reporting.realtime'));
      });
    });
